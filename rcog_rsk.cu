...
{	
	...
	char filename[59] = "../FRSC_KO/CUDA_KO/rcog/red_gup.dat";
	int outlet = 3;
...
/* Checks
	Everything works except for final<<<>>>. Determine what is wrong. Start with cuda error checking. Kernel call seems to not be launching. After you work this out, get math right.  
	Then time it. 
	Then re check the stream program to make sure it still works. If both are satisfactory, clean up for them to be presentable individually.
	At this point write some form of documentation and put away in a perminant final directory, perferably close to komogros/ This form of documentation could be a read me for all programs in the directory or a few pieces of paper describing everything from variables to process to coding techniques to error checking to mistakes to problem that occured to limitations of program to how this will be encorporated in the end product.
	From here you can do some time trials. 
	Then experiment with getting them faster. 
	Then combine streaming and nT0 massive parallelization. 
	Finally, encorporate with fft. May not need to happen at the same time. Cannot happen at the same time actually. Two functions I think will be best. Will have to do for all of the 8 channels. Keep in mind that you can hold lots of gpu memory for maybe 10 second of data and then loop through the stuff with both stats and fft so you are not reading the same basic memory twice. This is good. 
	After this clean it up and put some menus on it then you have a finished product.

Coding is done.

From here you need to write a shit ton of documentation. Mostly about how the next person can carry on the project, but also as a reflection and what could have been better. 
*/
